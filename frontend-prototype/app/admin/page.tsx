...
const views: [View, string][] = [["summary", "검토 요약"], ["evidence", "답변과 근거"], ["plan", "계획과 실행"], ["review", "담당자 검토"], ["report", "자료 내보내기"]];
    ...
    <div className="review-screen"><WorkspaceTopbar active="admin" saving={busy} />
      <div className="review-layout">
        <aside className="review-directory">
          <div className="review-directory-title"><span>상담 기록</span><span>1건</span></div>
        ...
        <section className="review-workspace" id="review-workspace">
          ...
          {!matches ? <div className="review-empty"><h1>찾는 상담이 없어요.</h1><p>검색어나 검토 상태를 바꿔주세요.</p><button onClick={() => { setQuery(""); setFilter("all"); }}>전체 상담 보기</button></div> : <>
            ...
            <div className="review-tabs" aria-label="상담 검토 내용" role="tablist">{views.map(([key, label], index) => <button key={key} id={`review-tab-${key}`} role="tab" aria-selected={view === key} aria-controls={`review-panel-${key}`} tabIndex={view === key ? 0 : -1} onKeyDown={(event) => moveView(event, index)} onClick={() => setView(key)}>{label}</button>)}</div>
            {!report ? <p className="review-empty">{error ? "기록을 다시 불러온 뒤 검토할 수 있습니다." : "기록을 불러오는 중입니다."}</p> : <div className="review-view" id={`review-panel-${view}`} role="tabpanel" aria-labelledby={`review-tab-${view}`} tabIndex={0} key={view}>
              {view === "summary" && <><div className="review-section-heading"><div><span>사업 현황</span><h2>상황과 계획을 살펴봅니다.</h2></div><p>답변을 펼치면 질문과 원문을 함께 확인할 수 있습니다.</p></div><div className="review-overview"><section>{report.evidence.filter(item => ["monthly_average_sales", "fixed_operating_costs", "operating_day_drop_reason", "improvement_plan", "execution_readiness", "seasonality_outlook"].includes(item.questionId)).map(item => <Evidence key={item.id} item={item} />)}</section><aside className="review-context"><h3>확인할 정보</h3><p>답변 {count}개 수집 · 미응답 {report.missing.length}개</p>{report.missing.length ? <ul>{report.missing.slice(0, 5).map(item => <li key={item.id}>{item.label}</li>)}</ul> : <p>답변 수집을 마쳤습니다. 금액·기간과 증빙을 확인해 주세요.</p>}{report.missing.length > 5 && <small>외 {report.missing.length - 5}개 항목</small>}<button onClick={() => setView("evidence")}>전체 답변 확인 →</button><hr /><h3>담당자 의견</h3><p className="review-note-preview">{record?.note || "아직 남긴 의견이 없습니다."}</p><button onClick={() => setView("review")}>검토 의견 남기기 →</button></aside></div><p className="review-boundary">본인 진술을 질문별로 정리한 자료입니다. 답변 수집은 증빙 확인이나 금융 평가를 의미하지 않습니다.</p></>}
              {view === "evidence" && <><div className="review-section-heading"><div><span>답변과 근거</span><h2>사장님의 말, 그대로.</h2></div><p>각 답변에는 검토서와 연결되는 고유 근거 번호가 있습니다.</p></div>{report.coverage.map(group => <section className="review-evidence-group" key={group.category}><header><h3>{group.category}</h3><span>{group.answered} / {group.total}</span></header>{report.evidence.filter(item => item.category === group.category).map(item => <Evidence key={item.id} item={item} />)}</section>)}</>}
              {view === "plan" && <><div className="review-section-heading"><div><span>계획과 실행</span><h2>다시 문을 여는 계획.</h2></div><p>인터뷰에서 확인한 목표와 실행 준비를 한곳에서 검토합니다.</p></div><div className="review-plan"><section><span>회복 목표</span><h3>월 영업일을 23일에서 29일까지</h3><p>{report.evidence.find(item => item.questionId === "improvement_plan")?.originalText ?? "아직 구체적인 개선 계획을 답하지 않았습니다."}</p><dl><div><dt>현재</dt><dd>월 {scenario.focus.baselineOperatingDays}일</dd></div><div><dt>목표</dt><dd>월 {scenario.focus.targetOperatingDays}일</dd></div><div><dt>기간</dt><dd>{scenario.focus.horizonMonths}개월</dd></div></dl></section><section><span>실행 준비</span><h3>예산과 남은 장애물</h3><p>{report.evidence.find(item => item.questionId === "execution_readiness")?.originalText ?? "아직 실행 준비 상태를 답하지 않았습니다."}</p><dl><div><dt>준비 예산</dt><dd>{scenario.focus.planBudgetWon.toLocaleString("ko-KR")}원</dd></div><div><dt>확인 방법</dt><dd>영업 장부</dd></div></dl></section></div><div className="review-plan-evidence">{report.evidence.filter(item => ["operating_day_drop_reason", "improvement_plan", "execution_readiness"].includes(item.questionId)).map(item => <Evidence key={item.id} item={item} />)}</div><p className="review-boundary">계획은 인터뷰 원문을 정리한 것이며, 실행 여부나 금융 지원 적합성을 판단한 결과가 아닙니다.</p></>}
              {view === "review" && <><div className="review-section-heading"><div><span>담당자 검토</span><h2>확인한 내용과 다음 할 일.</h2></div><p>저장한 의견과 점검 상태가 최종 검토서에 포함됩니다.</p></div><div className="review-editor"><section><h3>자료 점검</h3><p>직접 확인한 항목만 표시해 주세요.</p>{REVIEW_ITEMS.map(item => <label className="review-check" key={item}><input type="checkbox" disabled={busy} checked={checked.includes(item)} onChange={e => { edit(); setChecked(current => e.target.checked ? [...current, item] : current.filter(value => value !== item)); }} /><span>{item}</span><small>{checked.includes(item) ? "확인" : "대기"}</small></label>)}<p className="review-boundary">미확인 항목이 있어도 검토자료를 만들 수 있습니다.</p></section><section><label className="review-field">검토 상태<select value={disposition} disabled={busy} onChange={e => { edit(); setDisposition(e.target.value as ReviewDisposition); }}>{Object.entries(REVIEW_STATES).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label><label className="review-field">담당자 의견<textarea rows={7} maxLength={5000} value={note} disabled={busy} onChange={e => { edit(); setNote(e.target.value); }} placeholder="확인한 사실, 보완할 자료, 다음 상담에서 확인할 내용을 적어주세요." /></label><small>{note.length.toLocaleString()} / 5,000자 · 마지막 저장 {formatDate(record?.reviewUpdatedAt ?? null)}</small><div className="review-editor-actions"><button className="review-primary" onClick={saveReview} disabled={busy || !dirty}>{busy ? "저장 중…" : "검토 내용 저장"}</button>{dirty && <button onClick={() => { if (window.confirm("저장하지 않은 검토 변경을 버리고 서버의 기록을 불러올까요?")) { void refresh(true); setNotice(""); } }} disabled={busy}>저장된 검토 불러오기</button>}</div></section></div></>}
              {view === "report" && <><div className="review-section-heading"><div><span>최종 검토서</span><h2>상담을 위한 한 부의 기록.</h2></div><p>원문·미응답·담당자 의견을 함께 담습니다.</p></div><div className="review-export"><div><strong>{record?.completedAt ? "인터뷰 완료본" : "진행 중 자료"}</strong><p>{dirty ? "저장하지 않은 의견이 있습니다. 담당자 검토에서 먼저 저장해 주세요." : "내려받기 직전에 서버의 최신 저장 기록을 다시 확인합니다."}</p></div><button disabled={busy || dirty || !count} onClick={() => void exportReport("json")}>JSON 받기 ↓</button><button className="review-primary" disabled={busy || dirty || !count} onClick={() => void exportReport("print")}>인쇄 · PDF</button></div>{!count && <p className="review-boundary">저장된 답변이 있어야 내보낼 수 있습니다. 아래는 빈 검토서의 미리보기입니다.</p>}<ReviewDocument report={report} /></>}
            </div>}
