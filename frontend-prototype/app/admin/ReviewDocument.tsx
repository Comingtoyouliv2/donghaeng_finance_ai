import { REVIEW_STATES } from "../demo/record";
...
export default function ReviewDocument({ report }: { report: ReviewReport }) {
  return <article className="review-document">
    <header><span>동행금융 · 상담 검토자료</span><p>{report.stage === "COMPLETED" ? "인터뷰 완료본" : "진행 중 자료"} · {REVIEW_STATES[report.review.disposition]}</p><h1>{report.persona.businessName}</h1><p>{report.persona.borrowerName} 사장님 · {report.persona.industryLabel} · {report.scope}</p><small>자료 조회 {formatDate(report.preparedAt)} (한국 시간) · 답변 버전 {report.answerRevision} · 검토 버전 {report.review.revision}</small></header>
    ...
    <section><h2>03. 담당자 검토 의견</h2><p>{REVIEW_STATES[report.review.disposition]} · 저장 {formatDate(report.review.updatedAt)}</p><blockquote>{report.review.note || "저장된 검토 의견이 없습니다."}</blockquote></section>
    <section><h2>04. 전체 인터뷰와 원문 근거</h2>{report.evidence.map(item => <div className="document-answer" key={item.id}><h3>{String(item.number).padStart(2, "0")}. {item.label}</h3><p>{item.question}</p><blockquote>{item.originalText ?? "미응답"}</blockquote><small>{item.id}</small></div>)}</section>
